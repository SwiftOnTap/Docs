...
///
/// ``CardButtonStyle`` doesn't apply padding to its contents, so images, text,
/// and other content display edge-to-edge. A button with this style appears
/// partially translucent. When the user focuses on a card button, the button
/// animates up to a raised position with more opacity. ``CardButtonStyle`` also
/// applies the standard background colors for unfocused and focused states in
...
public struct CardButtonStyle : PrimitiveButtonStyle {
    ...
    ///
    /// The system calls this method for each ``Button`` instance in a view
    /// hierarchy in which ``CardButtonStyle`` is the current button style.
    ///
    /// - Parameter configuration: The properties of the button.
    public func makeBody(configuration: CardButtonStyle.Configuration) -> some View
...
open class UIHostingController<Content> : UIViewController where Content : View {
    ...
    @objc override dynamic open var preferredUserInterfaceStyle: UIUserInterfaceStyle { get }
...
extension View {
    ...
    /// Adds an action to perform when this view recognizes
    /// a long press gesture.
    @available(iOS, unavailable)
    @available(macOS, unavailable)
    @available(watchOS, unavailable)
    public func onLongPressGesture(minimumDuration: Double = 0.5, pressing: ((Bool) -> Void)? = nil, perform action: @escaping () -> Void) -> some View
